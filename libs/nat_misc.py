...
class NATSchedule(object):
    ...
    def cosine_schedule(t):
        return torch.cos(torch.tensor(t * math.pi * 0.5))
    ...
    @torch.no_grad()
    def generate_maskgit(self, gen_steps, _n_samples, nnet, decode_fn,
                 temperature, guidance_scale, **kwargs):
        device = self.device
        ...
        guidance_scales = torch.linspace(0, guidance_scale, gen_steps)

        for step in range(gen_steps):
            ratio = 1.0 * (step + 1) / gen_steps
            mask_ratio = self.cosine_schedule(ratio)

            # mask_ratio = manual_ratios[step] if step < gen_steps - 1 else 0
            # scaling temp
            annealed_temp = temperature * (1.0 - ratio)
            # annealed_temp = manual_temp[step] if step < gen_steps - 1 else 0
            # scaling cfg
            cfg_scale = guidance_scales[step]
            # samp_temp = manual_samp_temp[step]
            # sampling & scoring
            is_mask = (ids == self.mask_ind)
            logits = nnet(ids, **kwargs, scale=cfg_scale)
            sampled_ids = torch.distributions.Categorical(logits=logits).sample()
            logits = torch.log_softmax(logits, dim=-1)
            sampled_logits = torch.squeeze(
                torch.gather(logits, dim=-1, index=torch.unsqueeze(sampled_ids, -1)), -1)
            sampled_ids = torch.where(is_mask, sampled_ids, ids)
...
class GraphNATSchedule(object):
    ...
    @torch.no_grad()
    def generate_auto(self, gen_steps, _n_samples, nnet, decode_fn,
                 manual_ratios, manual_temp, manual_samp_temp, manual_cfg, **kwargs):
        device = self.device
    ...
    @torch.no_grad()
    def generate_auto_ids(self, gen_steps, _n_samples, nnet, decode_fn,
                 manual_ratios, manual_temp, manual_samp_temp, manual_cfg, **kwargs):
        device = self.device
