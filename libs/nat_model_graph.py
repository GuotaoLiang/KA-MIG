...
from .graph_layer import GraphAdapter
try:
    import xformers
    import xformers.ops
...
class UViTGraphAdapter(nn.Module):
    def __init__(self, img_size=16, in_chans=8, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4.,
                 ...
                 num_classes=None,
                 co_occurrence_graph_path=None,
                 replace_graph_path=None,
                 position_negative_codebook_graph_path=None,
                 gnn='gcn'):
        super().__init__()
        ...
        self.graph_layer = GraphAdapter(
            vocab_size=vocab_size,
            hidden_size=embed_dim,
            transformer_layers=depth + 1,
            co_occurrence_graph_path=co_occurrence_graph_path,
            replace_graph_path=replace_graph_path,
            position_negative_codebook_graph_path=position_negative_codebook_graph_path,
            gnn=gnn,
        )
    ...
    def forward(self, masked_ids, timesteps=None, context=None):
        ...
        # get graph prior knowledge
        mask = (masked_ids == self.codebook_size).unsqueeze(-1)

        word_embeddings = self.token_emb.word_embeddings.weight[:self.codebook_size]
        co_graph_word_embs_layers, re_graph_word_embs_layers, pos_neg_embs_layers = self.graph_layer(word_embeddings, masked_ids, context.reshape(-1), mask)
        
        
        if self.skip:
            skips = []
        for blk in self.in_blocks:
            x = x + re_graph_word_embs_layers.pop() + co_graph_word_embs_layers.pop() - pos_neg_embs_layers.pop()
            x = blk(x)
            if self.skip:
                skips.append(x)
